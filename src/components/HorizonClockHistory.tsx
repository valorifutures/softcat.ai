...
export default function HorizonClockHistory({ history, id, title, stance }: { history: ClockEvent[]; id: string; title: string; stance: Stance }) {
  ...
  return <section class="hz-clock-history" id="horizon-clock-history" aria-labelledby="horizon-history-title">
    ...
    <p class="hz-history-intro">Every recorded review, including decisions to keep the same window. Changing the outlook above compares assumptions. Evidence revisions appear in this record.</p>
    <div class="hz-history-entries">{entries.map(entry => <details key={`${id}-${stance}-${entry.id}`} class="hz-history-entry">
      <summary><time dateTime={entry.date}>{formatClockDate(entry.date)}</time><span class="hz-history-movement">{entry.movement.label}</span><span class="hz-history-window">{entry.record.timeframes[stance]}</span><span class="hz-history-open" aria-hidden="true"><span>Read +</span><span>Close −</span></span></summary>
