...
- `src/data/models.json` — array of AI model specs (pricing, context, reference notes). Hosted API prices can be null for unknown. `pricingStatus`, `pricingCheckedAt` and `pricingSource` record verification. Cost tools only use `hasVerifiedPrice` entries. Open weights do not imply free API pricing. **This file IS the model roster**: model_bot Job 1 refreshes prices/specs for whatever ids it contains; new models enter ONLY via Job 2's radar-gated proposal PRs (branch `model-bot/roster-proposals`, one open PR updated in place, placeholder reference metadata verified during review). Validate with `node scripts/validate-tools-data.mjs` (also runs in PR CI with `bot/tests/` pytest).
