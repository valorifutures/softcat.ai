...
merges with manually maintained reference notes in models.json,
and pushes if anything changed.

Auto-updated fields: contextK, inputPrice, outputPrice, multimodal
Manual fields (preserved): family, reasoning, description, strengths,
    openSource, released
...
NEW_MODEL_DEFAULTS = {
    ...
    "reasoning": False,
    "openSource": False,
    "description": "PLACEHOLDER - verify the model description and reference metadata before merge.",
    ...
    try:
        httpx.post(webhook, json={
            "content": f"**Model bot:** roster proposal PR for {names}\n"
                       f"Verify the placeholder metadata before merging: {pr_url}",
...
def propose_roster_pr(entries: list[dict]) -> str | None:
    ...
    if existing_pr:
        ...
        body = ("## Roster proposal (radar-gated)\n\n" + "\n".join(lines) +
                "\n\nReference notes are PLACEHOLDER. Verify the description, release date, "
                "weight licence and other metadata before merging. Trigger: model on OpenRouter AND featured on Radar AND "
