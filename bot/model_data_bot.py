...
# branch, clearly marked as proposals, and are edited during PR review.
    ...
    try:
        httpx.post(webhook, json={
            "content": f"**Model bot:** roster proposal PR for {names}\n"
                       f"Review the model identity and recorded data before merging: {pr_url}",
...
def propose_roster_pr(entries: list[dict]) -> str | None:
    ...
    if existing_pr:
        ...
        body = ("## Roster proposal (radar-gated)\n\n" + "\n".join(lines) +
                "\n\nEntries carry rosterProposal: true. Check model identity, price, context and any "
                "weight sources, then remove that marker before merging. Trigger: model on OpenRouter AND featured on Radar AND "
